    ...
    <main className="min-h-screen w-full bg-[rgb(var(--ink))] p-6">
      <div className="mx-auto max-w-2xl space-y-6">
        <header className="flex items-baseline justify-between">
          <div>
            <h1 className="text-[20px] text-[rgb(var(--ivory))]">
              Was it right?
            </h1>
            <p className="mt-1 text-[rgb(var(--muted))]">
              {data.totals.total} verdicts recorded across {data.sources.length}{" "}
        ...
        {data.totals.total === 0 ? (
          <p className="panel rounded p-4 text-[rgb(var(--muted))]">
            Nothing recorded yet. Mark whether a reading landed as you go, and
            this fills in. After fifty or so verdicts it starts telling you
            which parts of the software to trust.
          </p>
        ) : (
          <>
            <section className="panel rounded p-4">
              <div className="flex items-baseline gap-3">
                <span className="data text-[32px] text-[rgb(var(--brass))]">
                  {data.totals.score}%
                </span>
                <span className="text-[rgb(var(--muted))]">
                  {data.totals.landed} landed, {data.totals.partly} partly,{" "}
              ...
              {thin && (
                <p className="mt-2 border-l-2 border-[rgb(var(--vermilion))] pl-2 text-[rgb(var(--muted))]">
                  Too few verdicts for this to mean anything yet. Under about
                  thirty it moves several points on a single answer, so treat it
                  as a counter rather than a result.
                </p>
              )}
            </section>
                ...
                {data.sources.map((s) => (
                  <li key={s.source}>
                    ...
                    <div className="flex h-1.5 overflow-hidden rounded-full bg-[rgb(var(--hairline))]">
                      <span
                        style={{
                          width: `${(s.landed / s.total) * 100}%`,
                          background: "rgb(var(--lapis))",
                        }}
                      />
                      <span
                        style={{
                          width: `${(s.partly / s.total) * 100}%`,
                          background: "rgb(var(--brass))",
                        }}
                      />
                      <span
                        style={{
                          width: `${(s.missed / s.total) * 100}%`,
                          background: "rgb(var(--vermilion))",
                        }}
                      />
                    </div>
                  </li>
            ...
            {data.misses.length > 0 && (
              <section className="panel rounded p-4">
                ...
                <p className="mb-3 text-[rgb(var(--muted))]">
                  The only entries worth reading. A pattern here says which
                  module needs work.
                </p>
                <ul className="space-y-3">
                  {data.misses.map((m, i) => (
                    <li
                      key={i}
                      className="border-l-2 border-[rgb(var(--vermilion))] pl-3"
                    >
                      <span className="data text-[10px] text-[rgb(var(--muted))]">
                        {LABEL[m.source] ?? m.source} &middot;{" "}

...
export default function PlansPage() {
  ...
  const [form, setForm] = useState<{ gateway: string; fields: Record<string, string> } | null>(null);
      ...
      <div className="mx-auto max-w-5xl">
        <header className="mb-8">
          ...
          <p className="mt-1 text-[rgb(var(--muted))]">
            Every plan reads the same chart. What changes is how much of it you are shown.
          </p>
        </header>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {ORDER.map((tier) => {
            ...
            return (
              <section
                key={tier}
                className="panel flex flex-col rounded p-4"
                style={{ borderColor: tier === 'pro' ? 'rgb(var(--brass))' : undefined }}
              >
                <h2 className="text-[15px] text-[rgb(var(--ivory))]">{plan.label}</h2>
                <p className="data mt-0.5 text-[11px] text-[rgb(var(--brass))]">{PRICE_LABEL[tier]}</p>
                <p className="data text-[10px] text-[rgb(var(--muted))]">{plan.charts}</p>
                <p className="mt-2 text-[rgb(var(--muted))]">{plan.audience}</p>

                <ul className="mt-4 flex-1 space-y-1.5">
                  {plan.adds.map((line) => (
                    <li key={line} className="flex items-start gap-1.5 text-[rgb(var(--ivory))]">
                ...
                {paid ? (
                  ...
                  >
                    {busy === tier ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Pay with eSewa'}
                  </Button>
                ) : (
        ...
        <p className="mt-6 text-[11px] text-[rgb(var(--muted))]">
          Paid plans run for a fixed period and do not renew automatically. When the period ends the account
          returns to Free and nothing is charged again.
        </p>

        <a href="/dashboard" className="data mt-4 inline-block text-[11px] text-[rgb(var(--muted))] hover:text-[rgb(var(--brass))]">
